...
def test_make_cache_key(use_case_id: str) -> None:
    with override_options({"sentry-metrics.indexer.read-new-cache-namespace": False}):
        ...
        orgId = 1
        string = ":blah:blah"
        key = indexer_cache.make_cache_key(namespace, f"{use_case_id}:{orgId}:{string}")
...
def test_formatted_results(use_case_id: str) -> None:
    with override_options({"sentry-metrics.indexer.read-new-cache-namespace": False}):
        ...
        results = {indexer_cache.make_cache_key(namespace, k): v for k, v in values.items()}
        assert indexer_cache._format_results(namespace, list(values.keys()), results) == values
