...
def configure_sdk():
    ...
    if os.environ.get("SENTRY_DEVSERVICES_DSN") is not None:
        ...
        if server_dsn:
            ...
            sanitized_host = urlparse(f"http://{raw_host}").netloc  # make codeql happy
            host = "sentry.io" if sanitized_host.endswith("sentry.io") else sanitized_host
            logger.info(f"Server errors will be reported to project {project_id} on host '{host}'.")
        else:
